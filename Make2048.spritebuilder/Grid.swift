...
//  Make2048
//
//  Created by Iavor Dekov on 7/21/15.
...
class Grid: CCNodeColor {
    ...
    func spawnRandomTile() {
        var spawned = false
        while !spawned {
            // Int(Float(arc4random_uniform(1)) * Float(gridSize))
            let randomRow = Int(arc4random_uniform(UInt32(gridSize)))
            let randomColumn = Int(arc4random_uniform(UInt32(gridSize)))
